...
Each member is a **pre-built** :class:`~alcyoneus.core.graph.agent.Agent` (or
any :class:`~alcyoneus.core.graph.base_agent.BaseAgent` subclass) configured
independently by the caller.  ``SwarmAgent`` auto-generates
``transfer_to_<name>`` handoff tools and injects them into each member's
:class:`~alcyoneus.core.graph.tool_node.ToolNode` so the LLM can call them.
...
Example::
    ...
    from alcyoneus.prebuilt.agent.swarm import SwarmMemberConfig
...
from alcyoneus.prebuilt.tools.handoff import create_handoff_tool, is_handoff_tool
...
logger = logging.getLogger("alcyoneus.prebuilt.swarm")
...
class SwarmMemberConfig:
    ...
    Args:
        agent: A fully-configured :class:`~alcyoneus.core.graph.agent.Agent`
            ...
            :class:`~alcyoneus.core.graph.tool_node.ToolNode`; they are
...
class SwarmAgent[StateT: AgentState]:
    ...
    Each member is a **pre-built** :class:`~alcyoneus.core.graph.agent.Agent`
    ...
    * generates ``transfer_to_<name>`` handoff tools for each member's
      ...
      :class:`~alcyoneus.core.graph.tool_node.ToolNode`;
    * wires per-member conditional edges that inspect the last assistant
    ...
    def _inject_handoff_tools(self, name: str) -> None:
        ...
        :class:`~alcyoneus.core.graph.tool_node.ToolNode` is created for the
        handoff tools.  If it already has one, each handoff tool is added via
        :meth:`~alcyoneus.core.graph.tool_node.ToolNode.add_tool`.
