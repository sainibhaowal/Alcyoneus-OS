...
* a **store** (``QdrantStore`` or any :class:`~alcyoneus.storage.store.BaseStore`)
  that backs the knowledge base;
* a **pre-built agent** (``Agent`` or any :class:`~alcyoneus.core.graph.base_agent.BaseAgent`)
...
Example — no reranker::
    ...
    from alcyoneus.core.graph import Agent
    from alcyoneus.prebuilt.agent import RAGAgent
    from alcyoneus.storage import create_local_qdrant_store
    from alcyoneus.storage.store.embedding import OpenAIEmbedding
...
Example — with Cohere Rerank::
    ...
    from alcyoneus.prebuilt.agent.rag import CohereReranker
...
Example — fully local with CrossEncoder::
    ...
    from alcyoneus.prebuilt.agent.rag import CrossEncoderReranker
...
Example — parallel retrieval from multiple stores::

    from alcyoneus.storage import create_local_qdrant_store

    store1 = create_local_qdrant_store(path="./kb1", embedding=OpenAIEmbedding())
    store2 = create_local_qdrant_store(path="./kb2", embedding=OpenAIEmbedding())

    rag = RAGAgent(
        stores=[store1, store2],  # Parallel retrieval from multiple stores
        agent=Agent(model="gpt-4o-mini"),
        top_k=5,  # top_k per store
        merge_strategy="concat",  # or "deduplicate", "rank"
    )
    app = rag.compile()
"""
...
from alcyoneus.storage.store.store_schema import RetrievalStrategy
...
logger = logging.getLogger("alcyoneus.prebuilt.rag")
...
class RAGAgent[StateT: AgentState]:
    ...
    * ``store`` — the knowledge base (:class:`~alcyoneus.storage.store.BaseStore`)
      OR ``stores`` — multiple stores for parallel retrieval
    * ``agent`` — the LLM that generates the answer
        (:class:`~alcyoneus.core.graph.base_agent.BaseAgent`)
    * ``reranker`` — optional :class:`BaseReranker` for improved precision
    ...
    Args:
        ...
        stores: Optional list of stores for parallel retrieval. If provided,
            ``store`` is ignored. Retrieves from all stores in parallel.
        ...
        top_k: Candidates retrieved from the store (default ``5``).  With a
            ...
            candidates. When using multiple stores, this is per-store.
        ...
        retrieval_strategy: Vector search strategy passed to ``store.asearch``
            (default :attr:`~alcyoneus.storage.store.store_schema.RetrievalStrategy.SIMILARITY`).
        ...
        merge_strategy: Strategy for merging results from multiple stores.
            Options: "concat" (default), "deduplicate", "rank".
        parallel_retrieval: Enable parallel retrieval from multiple stores (default True).
    ...
    def __init__(  # noqa: PLR0913
        ...
        stores: list[BaseStore] | None = None,
        ...
        if store is None and stores is None:
            raise ValueError("Either 'store' or 'stores' must be provided")
        if store is not None and stores is not None:
            raise ValueError("Provide either 'store' or 'stores', not both")
        if top_k < 1:
            raise ValueError("top_k must be >= 1")
        if top_n < 1:
            raise ValueError("top_n must be >= 1")
        if merge_strategy not in ("concat", "deduplicate", "rank"):
            raise ValueError("merge_strategy must be 'concat', 'deduplicate', or 'rank'")

        self._stores = stores if stores is not None else [store]  # type: ignore
        self._agent = agent
    ...
    @property
    def _store(self):
        """Compatibility accessor returning the primary store.

        ``self._stores`` holds a list of stores; this property exposes the
        primary (first) store for code that predates multi-store support.
        """
        stores = self._stores
    ...
    def _make_retrieve_node(self):
        ...
        parallel_retrieval = self._parallel_retrieval
        ...
        async def _retrieve(state: AgentState, config: dict) -> AgentState:
            ...
            logger.debug(
                "RAGAgent RETRIEVE: query=%r top_k=%d stores=%d", query, top_k, len(stores)
            )
            ...
            if merge_strategy == "deduplicate":
                ...
                # Keep all docs but they'll be reranked if reranker is provided
                pass
            # "concat" is default - just use all docs as-is

            logger.debug(
                "RAGAgent RETRIEVE: got %d docs (merge_strategy=%s)", len(docs), merge_strategy
            )
    ...
    def compile(
        ...
        Returns:
            A compiled, invocable :class:`~alcyoneus.core.graph.CompiledGraph`.
        """
