"""
Input validation utilities for Alcyoneus OS.
...
Example:
    ...
    from alcyoneus.utils.validators import PromptInjectionValidator, register_default_validators
        ...
        async def validate(self, messages: list[Message]) -> bool:
            for msg in messages:
                if "bad_word" in msg.text():
                    from alcyoneus.utils.validators import ValidationError
...
from alcyoneus.utils.callbacks import BaseValidator
...
class PromptInjectionValidator(BaseValidator):
    ...
    Example:
        ```python
        from alcyoneus.utils.callbacks import CallbackManager
        from alcyoneus.utils.validators import PromptInjectionValidator
...
class MessageContentValidator(BaseValidator):
    ...
    Example:
        ```python
        from alcyoneus.utils.callbacks import CallbackManager
        from alcyoneus.utils.validators import MessageContentValidator
...
def register_default_validators(callback_manager: Any, strict_mode: bool = True) -> None:
    ...
    Example:
        ```python
        from alcyoneus.utils.validators import register_default_validators
        from alcyoneus.utils.callbacks import CallbackManager
