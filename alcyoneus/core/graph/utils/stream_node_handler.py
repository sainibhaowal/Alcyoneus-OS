...
from alcyoneus.core.graph.utils.stream_utils import check_non_streaming
from alcyoneus.core.graph.utils.utils import process_node_result
from alcyoneus.core.state import AgentState, Message
from alcyoneus.core.state.message_block import ErrorBlock
...
class StreamNodeHandler(BaseLoggingMixin):
    ...
    def __init__(
        ...
        Args:
            name: Unique identifier for the node within the graph.
            func: The function, ToolNode, or Agent to execute. Determines streaming behavior.
            execution_policy: Optional per-node execution policy (retry/cache/error/timeout).
        """
        self.name = name
        self.func = func
    ...
    async def _call_tools(
        ...
        if (
            ...
            from alcyoneus.prebuilt.tools.handoff import is_handoff_tool
    ...
    async def _call_normal_node(  # noqa: PLR0912, PLR0915
        ...
        try:
            ...
            recovery_result = (
                await callback_mgr.execute_on_error(context, input_data, e)
                if callback_mgr is not None
                else None
            )
