...
from alcyoneus.core.graph.edge import Edge
...
from alcyoneus.core.state.reducers import add_messages
    ...
    async def _execute_graph(  # noqa: PLR0912, PLR0915
        ...
        try:
            while current_node != END and step < max_steps:
                ...
                from alcyoneus.core.graph.command import Command

                if isinstance(result, Command):
                    if result.update:
                        for k, v in result.update.items():
                            if k == "messages":
                                msgs_to_add = v if isinstance(v, list) else [v]
                                state.context = add_messages(state.context, msgs_to_add)
                                messages.extend(msgs_to_add)
                            elif hasattr(state, k):
                                setattr(state, k, v)
                    if result.goto:
                        next_node = result.goto[0] if isinstance(result.goto, list) else result.goto
                    ...
                    # Apply tombstone directives to the accumulated message list so
                    # deleted messages do not surface in the final response.
                    from alcyoneus.core.state.remove_message import is_remove_message

                    if any(is_remove_message(m) for m in result):
                        state.context = add_messages(state.context, result)
                        removed_ids = {
                            d.message_id
                            for d in result
                ...
                if isinstance(result, dict):
                    ...
                    if new_messages:
                        ...
                        for nm in new_messages:
                            if nm.message_id not in existing_ids:
                                messages.append(nm)
                                existing_ids.add(nm.message_id)
                        logger.debug(
                            "Node '%s' returned %d messages, total messages now %d",
                            current_node,
                            len(new_messages),
            ...
            # Reconcile the returned messages with the final state context so
            # RemoveMessage tombstones (and REMOVE_ALL_MESSAGES) applied during
            # execution are reflected in the final response list. The state
            # context is the source of truth for ordering; any messages produced
            # during execution that are not part of the context are appended at
            # the end (in their original relative order).
            if state.context:
                context_ids = {m.message_id for m in state.context}
                extras = [m for m in messages if m.message_id not in context_ids]
                messages = list(state.context) + extras
            elif messages:
                # Context was fully cleared (e.g. REMOVE_ALL_MESSAGES or a
                # single-message tombstone). Drop stale messages from the result.
                messages = []
