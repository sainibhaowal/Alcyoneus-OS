...
class All:
    """Selector sentinel representing a wildcard/catch-all routing target.

    In conditional routing, a ``path_map`` entry keyed with ``"*"`` (or the
    ``All`` sentinel) matches any condition result that has no explicit mapping.
    This mirrors LangGraph's ``All`` selector semantics.

    Example:
        >>> from alcyoneus.core.graph.edge import All
        >>> path_map = {"urgent": "urgent_handler", All: "default_handler"}
        >>> # condition returning anything other than "urgent" routes to "default_handler"
    """
...
class Edge:
    ...
    def is_all_selector(self) -> bool:
        """Return True if this edge is a wildcard ``All``/``*`` conditional edge."""
        return self.condition_result in ("*", "All", "__all__")
