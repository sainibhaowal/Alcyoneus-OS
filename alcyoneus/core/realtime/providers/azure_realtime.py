"""Azure OpenAI Realtime provider client.

Implements the RealtimeClient protocol for Azure OpenAI Realtime API.
"""
...
class AzureRealtimeClient:
    """RealtimeClient implementation for Azure OpenAI Realtime API."""
    ...
    async def connect(self, config: RealtimeConfig, resume_handle: str | None = None) -> None:
        if not self._api_key or not self._endpoint or not self._deployment:
            raise ValueError(
                "Azure credentials required: AZURE_OPENAI_API_KEY, AZURE_OPENAI_ENDPOINT, AZURE_OPENAI_DEPLOYMENT"  # noqa: E501
            )

        self._config = config
        url = f"wss://{self._endpoint}/openai/realtime?api-version={self._api_version}&deployment={self._deployment}"
        ...
        logger.info("Azure Realtime connected: %s", self._deployment)
    ...
    def _normalize_message(self, data: dict) -> list[RealtimeEvent]:
        ...
        if msg_type == "response.audio.delta":
            ...
            err = data.get("error", {})
            from alcyoneus.core.realtime.base import ErrorEvent
