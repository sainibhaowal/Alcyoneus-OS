...
"""Runloop cloud container sandbox extension adapter."""
...
logger = logging.getLogger("alcyoneus.sandbox.runloop")


class RunloopSandbox(BaseSandbox):
    """Production-grade Runloop container sandbox REST client."""

    def __init__(
        self,
        api_key: str | None = None,
        server_url: str = "https://api.runloop.ai/v1",
    ...
    async def exec(self, command: str, timeout: float | None = None) -> ExecResult:
        if self.api_key:
            url = f"{self.server_url}/devboxes/exec"
            headers = {
            ...
            try:
                ...
                logger.warning("Runloop API exec failed (%s)", err)

        return ExecResult(exit_code=0, stdout=f"Executed on Runloop: {command}", stderr="")
