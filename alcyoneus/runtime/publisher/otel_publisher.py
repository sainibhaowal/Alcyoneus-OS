"""OtelPublisher — fire-and-forget OTEL backend for Alcyoneus OS.
...
Requires: pip install 'alcyoneus[otel]'
"""
...
logger = logging.getLogger("alcyoneus.otel")
...
def _guard() -> None:
    ...
    try:
        ...
        raise ImportError(
            "OpenTelemetry is required for tracing. Install with: pip install 'alcyoneus[otel]'"
        ) from exc
...
def _provider_system_name(raw_provider: str) -> str:
    """Map an alcyoneus provider string to the OTEL gen_ai.system value."""
...
class OtelPublisher(BasePublisher):
    ...
    Usage (minimal — STANDARD level, global TracerProvider):
        from alcyoneus.runtime.publisher.otel_publisher import setup_tracing
    ...
    def _get_tracer(self) -> Tracer:
        if self._tracer is None:
            ...
            self._tracer = self._tracer_arg or trace.get_tracer("alcyoneus")
        return self._tracer
    ...
    def _on_graph_start(self, event: EventModel) -> None:
        ...
        if lifecycle == "validation_rejected":
            ...
            span.set_attribute(GEN_AI_SYSTEM, "alcyoneus")
    ...
    def _on_llm_start(self, event: EventModel) -> None:
        ...
        if model:
            ...
            span.set_attribute(GRAPH_MODEL, model)  # keep alcyoneus-namespaced copy
