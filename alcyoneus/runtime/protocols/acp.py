...
class ACPMessage(BaseModel):
    ...
    context: MessageContext = Field(
        default_factory=MessageContext,  # type: ignore[arg-type]
...
class ACPProtocol:
    ...
    def create_error(
        ...
        **kwargs: Any,
        ...
        context = MessageContext()  # type: ignore[call-arg]
    ...
    def create_heartbeat(sender_id: str, recipient_id: str = "*") -> ACPMessage:
        ...
        """
        return ACPMessage(  # type: ignore[call-arg]
...
class ACPClient:
    ...
    async def _stream_task(
        ...
        while True:
            try:
                response = await asyncio.wait_for(self.transport.receive(), timeout=self.timeout)  # type: ignore[union-attr]
...
class ACPServer:
    ...
    def __init__(self, agent_id: str):
        self.agent_id = agent_id
        self.handlers: dict[str, callable] = {}  # type: ignore[valid-type]
        self._running = False

    def register_handler(self, action: str, handler: callable) -> None:  # type: ignore[valid-type]
