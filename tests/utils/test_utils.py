...
from alcyoneus.core.state.reducers import append_items
from alcyoneus.utils.background_task_manager import BackgroundTaskManager
from alcyoneus.utils.id_generator import (
    AsyncIDGenerator,
    BigIntIDGenerator,
    DefaultIDGenerator,
    HexIDGenerator,
...
from alcyoneus.utils.logging import logger as alcyoneus_logger
from alcyoneus.core.state.message import Message, TokenUsages, generate_id
from alcyoneus.core.state.message_block import TextBlock, ToolResultBlock
...
class TestLogging:
    ...
    def test_alcyoneus_logger_exists(self):
        """Test that alcyoneus logger is available."""
        assert alcyoneus_logger is not None
        assert alcyoneus_logger.name == "alcyoneus"

    def test_alcyoneus_logger_has_nullhandler(self):
        """Test that alcyoneus logger has NullHandler by default."""
        # The alcyoneus logger should have at least one handler (NullHandler)
        assert len(alcyoneus_logger.handlers) >= 1
        # Check that at least one handler is a NullHandler
        has_null_handler = any(
            isinstance(h, logging.NullHandler) for h in alcyoneus_logger.handlers
        )
        assert has_null_handler

    def test_user_can_configure_logging(self):
        """Test that users can configure logging for alcyoneus."""
        # Get the alcyoneus logger
        ...
        try:
            ...
            # Test that it works
            test_logger = logging.getLogger("alcyoneus.test")
            test_logger.debug("Test message")
            
            # Check output
            output = string_stream.getvalue()
            assert "DEBUG - alcyoneus.test: Test message" in output
    ...
    def test_module_loggers_work(self):
        ...
        # Create a module logger (as would be done in alcyoneus modules)
        module_logger = logging.getLogger("alcyoneus.test_module")
        
        # It should inherit from alcyoneus logger
        assert module_logger.parent.name == "alcyoneus"
        
        # Save state
        alcyoneus_logger_handlers = alcyoneus_logger.handlers.copy()
        ...
        try:
            ...
            alcyoneus_logger.addHandler(handler)
            alcyoneus_logger.setLevel(logging.INFO)
            ...
            # Restore
            alcyoneus_logger.handlers.clear()
            for h in alcyoneus_logger_handlers:
                alcyoneus_logger.addHandler(h)

    def test_logging_with_no_user_configuration(self):
        """Test that logging doesn't raise errors when user doesn't configure."""
        # Create a logger
        test_logger = logging.getLogger("alcyoneus.unconfigured")
    ...
    def test_configure_logging_with_all_custom_params(self):
        ...
        try:
            ...
            # Verify output
            output = string_stream.getvalue()
            assert "alcyoneus.test - WARNING: warning message" in output
    ...
    def test_default_configuration_on_import(self):
        ...
        # The alcyoneus logger should exist and have a NullHandler
        logger = logging.getLogger("alcyoneus")
    ...
    def test_logger_hierarchy(self):
        ...
        # Module logger should propagate to alcyoneus logger
        ...
        try:
            # Configure alcyoneus logger to capture output
            log_output = StringIO()
            ...
            assert "alcyoneus.test_module" in output
