...
from alcyoneus.core.graph import ToolNode, CompiledGraph
from alcyoneus.prebuilt.agent.react import ReactAgent, _make_should_use_tools, _should_use_tools
...
class TestReactAgent:
    ...
    def test_init_with_state(self):
        ...
        state = AgentState()
        with patch("alcyoneus.prebuilt.agent.react.Agent", FakeManagedAgent):
    ...
    def test_compile_forwards_media_store_and_shutdown_timeout(self):
        ...
        with patch(
            "alcyoneus.prebuilt.agent.react.StateGraph.compile",
            autospec=True,
    ...
    def test_constructor_builds_tool_node_from_client_only(self):
        ...
        with patch("alcyoneus.prebuilt.agent.react.Agent", FakeManagedAgent), patch(
            "alcyoneus.prebuilt.agent.react.ToolNode",
            FakeToolNode,
        ):
            react_agent = ReactAgent[AgentState](
                model="fake-model",
