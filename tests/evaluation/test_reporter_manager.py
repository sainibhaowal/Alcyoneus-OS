...
from alcyoneus.qa.evaluation.eval_result import CriterionResult, EvalCaseResult, EvalReport
...
def test_run_all_executes_enabled_reporters_and_generates_paths(tmp_path):
    ...
    with patch("alcyoneus.qa.evaluation.reporters.console.ConsoleReporter") as console_cls, patch(
        "alcyoneus.qa.evaluation.reporters.json.JSONReporter"
    ) as json_cls, patch("alcyoneus.qa.evaluation.reporters.html.HTMLReporter") as html_cls, patch(
        "alcyoneus.qa.evaluation.reporters.json.JUnitXMLReporter"
    ) as junit_cls:
...
def test_run_all_records_directory_creation_error(monkeypatch, tmp_path):
    ...
    with patch("alcyoneus.qa.evaluation.reporters.console.ConsoleReporter") as console_cls:
        console_inst = MagicMock()
        console_cls.return_value = console_inst
        out = manager.run_all(_sample_report())
